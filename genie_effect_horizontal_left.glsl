...
#define NO_REPEAT
    ...
    gl_FragColor = texture2D(u_texture_0, vec2(v_texcoord.x + translation_animation_progress, (v_texcoord.y - bottom) / (top - bottom)));
    if (v_texcoord.y > top || v_texcoord.y < bottom) {
        gl_FragColor = vec4(0.0);
    }
    
#ifdef NO_REPEAT
    if (v_texcoord.x + translation_animation_progress > 1.0) {
        gl_FragColor = vec4(0.0);
    }
#endif
}
